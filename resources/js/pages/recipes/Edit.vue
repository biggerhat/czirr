...
import type { Recipe, Cuisine, RecipeTag, RecipeDifficulty } from '@/types/recipes';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.recipe.name, href: `/recipes/${props.recipe.id}` },
    { title: 'Edit' },
];
...
const prepTime = ref(props.recipe.prep_time);
const cookTime = ref(props.recipe.cook_time);
const servings = ref(props.recipe.servings);
const imageUrl = ref(props.recipe.image_url ?? '');
const sourceUrl = ref(props.recipe.source_url ?? '');
const cuisineId = ref<string>(props.recipe.cuisine_id ? String(props.recipe.cuisine_id) : '');
const difficulty = ref<RecipeDifficulty>(props.recipe.difficulty);
const selectedTagIds = ref<number[]>(props.recipe.tags?.map(t => t.id) ?? []);
...
async function save() {
    ...
    try {
        const response = await fetch(`/recipes/${props.recipe.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
        ...
        <div class="flex h-full flex-1 flex-col gap-4 p-4 items-center">
            <form @submit.prevent="save" class="w-full max-w-2xl space-y-6">
                ...
                <div class="flex gap-3">
                    <Button type="button" variant="outline" @click="router.visit(`/recipes/${recipe.id}`)">
                        Cancel
                    </Button>
